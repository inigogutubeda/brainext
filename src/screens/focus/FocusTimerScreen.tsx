...
export function FocusTimerScreen({ navigation }: Props) {
  ...
  const handleStop = () => {
    Alert.alert("¬øSalir del modo foco?", "Perder√°s el progreso de esta sesi√≥n.", [
      { text: "Cancelar" },
  ...
  if (isComplete) {
    return (
      <SafeAreaView className="flex-1 bg-cream items-center justify-center px-6">
        ...
        <Text className="text-2xl font-bold text-stone-900 mb-2">¬°Sesi√≥n completada!</Text>
        ...
        {breaks.length > 0 && (
          <Text className="text-stone-400 text-sm text-center mb-8">
            {breaks.length} {breaks.length === 1 ? "interrupci√≥n" : "interrupciones"} registradas.
          </Text>
        )}
    ...
    <SafeAreaView className="flex-1 bg-cream">
      <View className="flex-1 px-6 pt-6">
        <TouchableOpacity onPress={() => navigation.goBack()} className="mb-6">
          <Text className="text-stone-400 text-sm">‚Üê Volver</Text>
        </TouchableOpacity>
        ...
        <View className="flex-1 items-center justify-center">
          ...
          {breaks.filter((b) => b.reason).length > 0 && (
            <Text className="text-xs text-stone-400 mb-4">
              {breaks.filter((b) => b.reason).length} {breaks.filter((b) => b.reason).length === 1 ? "interrupci√≥n" : "interrupciones"}
            </Text>
          )}
      ...
      <Modal visible={showAccountability} transparent animationType="slide">
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-white rounded-t-3xl px-6 pt-6 pb-10">
            ...
            <Text className="text-stone-500 mb-6">¬øQu√© pas√≥?</Text>
            {[
              { reason: "distracted" as BreakReason, label: "Me distraje", emoji: "Ì†ΩÌ∏Ö" },
              { reason: "necessary" as BreakReason, label: "Era necesario", emoji: "‚úÖ" },
              { reason: "lost" as BreakReason, label: "Me perd√≠", emoji: "Ì†ºÌºÄ" },
            ].map((opt) => (
              <TouchableOpacity
                key={opt.reason}
